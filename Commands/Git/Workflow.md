...
### 1.0 Configurar Encoding UTF-8 (OBRIGATÓRIO - Primeira vez)

**IMPORTANTE**: Configure o encoding UTF-8 antes de fazer qualquer commit para evitar mojibakes (caracteres mal codificados).
...
# Configurar PowerShell para UTF-8 (Windows)
...
git config --list | Select-String -Pattern "i18n|encoding"
```

**Nota**: Execute estes comandos uma vez por máquina. Eles garantem que commits com acentos e caracteres especiais sejam salvos corretamente.
...
- **IMPORTANTE**: Use UTF-8 para evitar mojibakes (caracteres mal codificados)
  - Verifique se o encoding está configurado (seção 1.0)
  - Se aparecerem caracteres como "Ã§" ou "Ã£" nos commits, há problema de encoding
...
### "Commits com mojibakes (caracteres mal codificados como Ã§, Ã£)"

**Causa**: Encoding não configurado corretamente antes dos commits.

**Prevenção**:
```bash
# Execute a seção 1.0 (Configurar Encoding UTF-8) antes de fazer commits
```

**Correção (se commits ainda não foram para produção)**:
```bash
# 1. Fazer rebase interativo dos últimos N commits
git rebase -i HEAD~N  # N = número de commits com problema

# 2. Para cada commit, mudar 'pick' para 'reword' (ou 'r')
# 3. Reescrever a mensagem com caracteres corretos
# 4. Salvar e fechar o editor

# 5. Se já foi feito push, fazer force push (CUIDADO!)
git push --force-with-lease origin nome-do-branch
```

**⚠️ ATENÇÃO**: 
- **NUNCA** faça force push na `main` se outros desenvolvedores estão trabalhando
- Force push reescreve o histórico e pode causar problemas para outros
- Se os commits já estão na `main` em produção, considere deixar como está e garantir que próximos commits sejam corretos

**Correção (se commits já estão na main)**:
- Deixe os commits como estão (já estão em produção)
- Garanta que próximos commits sejam feitos com encoding correto
- Documente o problema se necessário
...
### Início do Dia
- [ ] Configurei encoding UTF-8 (se primeira vez - seção 1.0)
