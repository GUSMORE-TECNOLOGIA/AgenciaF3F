-- Fix: filtro inteligente AND com contrato_assinado + nome.
-- 1) contrato_assinado: suportar operador 'contains' com ILIKE e igualdade case-insensitive.
-- 2) Garante que AND entre condições relacionais e nome funcione corretamente.
    ...
    CASE v_field
      ...
      WHEN 'contrato_assinado' THEN
        IF v_val IS NOT NULL AND v_val <> '' THEN
          ...
          ELSE
            v_frag := format('EXISTS (SELECT 1 FROM cliente_contratos cc WHERE cc.cliente_id = c.id AND cc.deleted_at IS NULL AND LOWER(cc.contrato_assinado) = LOWER(%L))', v_val);
          END IF;
        END IF;
      WHEN 'tem_plano' THEN
        IF (v_val = 'true' OR v_val = '1') THEN
...
COMMENT ON FUNCTION list_clientes_filtrados(jsonb, int, int) IS 'Lista clientes com filtros inteligentes; contrato_assinado suporta contains (ILIKE) e equals (case-insensitive); respeita visibilidade (responsavel ou admin).';
