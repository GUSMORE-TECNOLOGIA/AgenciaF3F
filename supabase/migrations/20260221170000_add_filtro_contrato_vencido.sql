-- Filtro inteligente: campo "Contrato vencido" (Sim/Não).
-- Considera contrato vencido quando existe cliente_contratos com data_fim <= hoje.
...
COMMENT ON FUNCTION list_clientes_filtrados(jsonb, int, int) IS 'Lista clientes com filtros inteligentes; contrato_vencido (data_fim <= hoje); contrato_assinado; respeita visibilidade (responsavel ou admin).';
