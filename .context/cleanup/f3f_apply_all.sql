...
CREATE POLICY "clientes_select_responsavel" ON clientes FOR SELECT USING (
...
CREATE POLICY "clientes_insert_responsavel" ON clientes FOR INSERT WITH CHECK (
...
CREATE POLICY "ocorrencia_grupos_select" ON ocorrencia_grupos FOR SELECT USING (
  (responsavel_id = auth.uid() AND is_active = true)
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);

DROP POLICY IF EXISTS "ocorrencia_grupos_insert" ON ocorrencia_grupos;
CREATE POLICY "ocorrencia_grupos_insert" ON ocorrencia_grupos FOR INSERT WITH CHECK (
  responsavel_id = auth.uid()
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);

DROP POLICY IF EXISTS "ocorrencia_grupos_update" ON ocorrencia_grupos;
CREATE POLICY "ocorrencia_grupos_update" ON ocorrencia_grupos FOR UPDATE USING (
  responsavel_id = auth.uid()
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);
...
CREATE POLICY "ocorrencia_tipos_select" ON ocorrencia_tipos FOR SELECT USING (
  (responsavel_id = auth.uid() AND is_active = true)
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);

DROP POLICY IF EXISTS "ocorrencia_tipos_insert" ON ocorrencia_tipos;
CREATE POLICY "ocorrencia_tipos_insert" ON ocorrencia_tipos FOR INSERT WITH CHECK (
  responsavel_id = auth.uid()
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);

DROP POLICY IF EXISTS "ocorrencia_tipos_update" ON ocorrencia_tipos;
CREATE POLICY "ocorrencia_tipos_update" ON ocorrencia_tipos FOR UPDATE USING (
  responsavel_id = auth.uid()
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);
...
CREATE POLICY "equipe_membros_select" ON equipe_membros FOR SELECT USING (
  (responsavel_id = auth.uid() OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin'))
  AND deleted_at IS NULL
);

DROP POLICY IF EXISTS "equipe_membros_insert" ON equipe_membros;
CREATE POLICY "equipe_membros_insert" ON equipe_membros FOR INSERT WITH CHECK (
  responsavel_id = auth.uid()
  OR EXISTS (SELECT 1 FROM usuarios u WHERE u.id = auth.uid() AND u.role = 'admin')
);
